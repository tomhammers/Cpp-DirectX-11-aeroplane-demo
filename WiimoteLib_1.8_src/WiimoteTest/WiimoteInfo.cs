...
namespace WiimoteTest
{
	public partial class WiimoteInfo : UserControl
	{
		...
		private void UpdateWiimoteChanged(WiimoteChangedEventArgs args)
		{
            ...
            aFile = new FileStream("../../../../WiiMoteData.txt", FileMode.OpenOrCreate);
            sw = new StreamWriter(aFile);

            sw.WriteLine("a " + ws.ButtonState.A);
            sw.WriteLine("r " + ws.AccelState.Values.X.ToString());
            sw.WriteLine("p " + ws.AccelState.Values.Y.ToString());

            sw.Close(); // close filestream
